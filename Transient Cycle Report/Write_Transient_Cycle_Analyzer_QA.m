...
PM = 0; NOx = 0; N2O = 0; NMHC = 0; NOx_NMHC = 0; CO = 0;
...
try
    ...
    sql = sprintf('SELECT b.PM, b.NOx, b.N2O, b.NMHC, b.NOx_NMHC, b.CO FROM engine AS a, tblFEL AS b WHERE (a.FEL = b.FEL_Name OR a.FEL = b.FEL_Code) AND a.control = ''%s''',num2str(engineID));   
    odbcCOM = System.Data.Odbc.OdbcCommand(sql, odbcCN);
    res = odbcCOM.ExecuteReader();
    res.Read();    
    if res.FieldCount == 6,
        ...
        if ~isa(res.GetValue(2),'System.DBNull'), N2O = double(res.GetValue(2)); end
        if ~isa(res.GetValue(3),'System.DBNull'), NMHC = double(res.GetValue(3)); end
        if ~isa(res.GetValue(4),'System.DBNull'), NOx_NMHC = double(res.GetValue(4)); end
        if ~isa(res.GetValue(5),'System.DBNull'), CO = double(res.GetValue(5)); end
    end
...
species_FEL = [0 CO NOx N2O 0 0 NMHC NOx_NMHC];
    ...
    if isKey(datastream,name)
        ...
        if datastream('Options').Part_1065.IsOn
           % Set the Range 
            ...
            drift_pct = (datastream(name).ModeCompositeData.Part1065BrakeSpecificMass(ModeNumber) - ...
            ...
            if (datastream(name).ModeCompositeData.Part1065BrakeSpecificMass(ModeNumber) - datastream(name).ModeCompositeData.BrakeSpecificMass(ModeNumber)) == 0 && ...
                datastream(name).ModeCompositeData.BrakeSpecificMass(ModeNumber) == 0
                    set(Range(Excel,[specie '_Analyzer_Drift_Pct']),'Value','0');
                    set(Range(Excel,[specie '_Analyzer_Drift_Pct']),'NumberFormat','0"*"')
                    set(Range(Excel,'Analyzer_Drift_Pct_Explanation'),'Value','* Both the corrected and raw Brake Specific Mass are zero.');
            elseif isnan(drift_pct) || isempty(drift_pct)
                % Do nothing
            else
                set(Range(Excel,[specie '_Analyzer_Drift_Pct']),'Value',drift_pct);
            end

            % Compare the Drift to the applicable emissions standard (FEL)
            % if it exists
            if species_FEL(row) ~= 0
                drift_pct_FEL = (datastream(name).ModeCompositeData.Part1065BrakeSpecificMass(ModeNumber) - ...
                datastream(name).ModeCompositeData.BrakeSpecificMass(ModeNumber))/species_FEL(row);                
                if ~isnan(drift_pct_FEL) && ~isempty(drift_pct_FEL)
                    set(Range(Excel,[specie '_Analyzer_Drift_Pct_FEL']),'Value',drift_pct_FEL);
                end                
            end            
            ...
            drift_pct_86 = (datastream(name).ModeCompositeData.Part86BrakeSpecificMass(ModeNumber) - ...
                datastream(name).ModeCompositeData.BrakeSpecificMass(ModeNumber))/datastream(name).ModeCompositeData.BrakeSpecificMass(ModeNumber);
            if (datastream(name).ModeCompositeData.Part86BrakeSpecificMass(ModeNumber) - datastream(name).ModeCompositeData.BrakeSpecificMass(ModeNumber)) == 0 && ...
                datastream(name).ModeCompositeData.BrakeSpecificMass(ModeNumber) == 0
                    set(Range(Excel,[specie '_Analyzer_Drift_Pct']),'Value','0');
                    set(Range(Excel,[specie '_Analyzer_Drift_Pct']),'NumberFormat','0"*"');
                    set(Range(Excel,'Analyzer_Drift_Pct_Explanation'),'Value','* Both the corrected and raw Brake Specific Mass are zero.');
            elseif isnan(drift_pct_86) || isempty(drift_pct_86)
                % Do nothing
            else
                set(Range(Excel,[specie '_Analyzer_Drift_Pct']),'Value',drift_pct_86);
            end 
        ...
        if datastream('Options').Part_1065.IsOn % Part 1065
            ...
            if (NOx_NMHC_1065_BSM - NOx_NMHC_uncorrected_BSM) == 0 && NOx_NMHC_uncorrected_BSM == 0
                set(Range(Excel,'NOx_NMHC_Analyzer_Drift_Pct'),'Value','0');
                set(Range(Excel,'NOx_NMHC_Analyzer_Drift_Pct'),'NumberFormat','0"*"');
                set(Range(Excel,'Analyzer_Drift_Pct_Explanation'),'Value','* Both the corrected and raw Brake Specific Mass are zero.');
            elseif isnan(NOx_NMHC_1065_Drift) || isempty(NOx_NMHC_1065_Drift)
                % Do nothing
            else
            ...
            if (NOx_NMHC_86_BSM - NOx_NMHC_uncorrected_BSM) == 0 && NOx_NMHC_uncorrected_BSM == 0
                set(Range(Excel,'NOx_NMHC_Analyzer_Drift_Pct'),'Value','0');
                set(Range(Excel,'NOx_NMHC_Analyzer_Drift_Pct'),'NumberFormat','0"*"');
                set(Range(Excel,'Analyzer_Drift_Pct_Explanation'),'Value','* Both the corrected and raw Brake Specific Mass are zero.');
            elseif isnan(NOx_NMHC_86_Drift) || isempty(NOx_NMHC_86_Drift)
                % Do nothing
            else
                set(Range(Excel,'NOx_NMHC_Analyzer_Drift_Pct'),'Value',NOx_NMHC_1065_Drift);
            end             
