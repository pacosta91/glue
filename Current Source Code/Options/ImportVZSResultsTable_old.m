function [datastream, options] = ImportVZSResultsTable(odbcCN, SQL1, SQL3eavt, SQL3ebr, datastream, options)
...
while (res.Read())
    ...
    if datastream.isKey(Writer)
        ...
        for k = 1:size(VZSPhrasing,1)
            try
                if VZSPhrasing{k,1}
                    ...
                    Ambient.(VZSPhrasing{k,2}) = double(res.GetValue(VZSPhrasing{k,3}));
                end
            catch %#ok
        ...
        if ~(VZS.RawPreTestSpan==0) && isfield(VZS,'CorrectedPostTestSpan')
            VZS.RawPostTestZero = (VZS.CorrectedPostTestZero-VZS.ReferenceZero)/(VZS.ReferenceConcentration-VZS.ReferenceZero)*(VZS.RawPreTestSpan-VZS.RawPreTestZero)+VZS.RawPreTestZero;
            VZS.RawPostTestSpan = (VZS.CorrectedPostTestSpan-VZS.ReferenceZero)/(VZS.ReferenceConcentration-VZS.ReferenceZero)*(VZS.RawPreTestSpan-VZS.RawPreTestZero)+VZS.RawPreTestZero;
            AverageZero = (VZS.RawPreTestZero+VZS.RawPostTestZero)/2;
            AverageSpan = (VZS.RawPreTestSpan+VZS.RawPostTestSpan)/2;
            Ambient.PreTest = (Ambient.Part86PreTest-VZS.ReferenceZero)/(VZS.ReferenceConcentration-VZS.ReferenceZero)*(VZS.RawPreTestSpan-VZS.RawPreTestZero)+VZS.RawPreTestZero;
            Ambient.PostTest = (Ambient.Part86PostTest-VZS.ReferenceZero)/(VZS.ReferenceConcentration-VZS.ReferenceZero)*(VZS.RawPreTestSpan-VZS.RawPreTestZero)+VZS.RawPreTestZero;
            Ambient.Average = (Ambient.PreTest+Ambient.PostTest)/2;
            Ambient.Part1065PreTest = VZS.ReferenceZero+(VZS.ReferenceConcentration-VZS.ReferenceZero)*(Ambient.PreTest-AverageZero)/(AverageSpan-AverageZero);
            Ambient.Part1065PostTest = VZS.ReferenceZero+(VZS.ReferenceConcentration-VZS.ReferenceZero)*(Ambient.PostTest-AverageZero)/(AverageSpan-AverageZero);
            Ambient.Part1065Average = (Ambient.Part1065PreTest+Ambient.Part1065PostTest)/2;
        end
        if (isempty(tempChannel.Ambient) || tempChannel.Ambient.Range > Range) && ~(VZS.RawPreTestSpan==0)
            tempChannel.Ambient = Ambient;
        end
        if ~(VZS.RawPreTestSpan==0) && isfield(VZS,'CorrectedPostTestSpan')
            tempChannel.VZS(Range) = VZS;
        elseif ~(VZS.RawPreTestSpan==0) || (isfield(VZS,'CorrectedPostTestSpan') && ~(VZS.CorrectedPostTestSpan==0))
