...
% EDS added 2/1/2017 - Note that the AmbientConc is set to 0 and the
% PhaseNum to 1 in cases when the test EXISTS in the Bag_Read_Details table
% but the values are NULL. They are -999 when they don't apply (i.e., do
% NOT exist in the Bag_Read_Details table OR they exist but are not a
% ModAmbBag procedure.

% SQL3eavt = ['a.*, ISNULL(b.AmbientConc, -999) AS AmbientConc, ISNULL(b.Phase_Num, -999) AS PhaseNum FROM tblVZS_Results AS a ' ...
%     'LEFT OUTER JOIN Bag_Read_Details AS b ON (a.Test_Number = b.Test_Number AND a.Analyzer = b.Gas AND a.Bench = 0 ' ...
%     'AND b.ProcedureID = ''ModAmbBag'' AND a.Range = b.Range) ' ...
%     'WHERE a.Test_Number = ' num2str(TestNumber) ' UNION SELECT *, AmbientConc = -999, PhaseNum = -999 FROM tblVZS_Results ' ...
%     'WHERE Bench <> 0 AND Test_Number = ' num2str(TestNumber) ' ORDER BY Analyzer, Range DESC, PhaseNum ASC'];
...
[datastream, options] = ImportVZSResultsTable(odbcCN, SQL1, SQL3eavt, SQL3ebr, datastream, options, TestNumber);
