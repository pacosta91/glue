function [datastream, options] = ImportVZSResultsTable(odbcCN, SQL1, SQL3eavt, SQL3brr, SQL3ebr, datastream, options, testnumber)
...
% ES added 3/1/2017
% Execute the Bag_Read_Details query
...
while (res.Read())
    ...
    specie = char(res.GetValue(1))
    Phase = res.GetValue(2);
    Range = res.GetValue(3);
    RangeSize = res.GetValue(4);
    Concentration = res.GetValue(5)

    Writer = [specie '_Bag_Dilute']
    Writer = strrep(Writer,'(','_'); % These two lines insure good behavior of channel name processor
    Writer = strrep(Writer,')','')
    ...
    if datastream.isKey(Writer)
        ...
        % Concentration
        if ~isempty(tempChannel.Ambient) && ~isempty(tempChannel.Ambient.BagRead) && ~isempty(tempChannel.Ambient.BagRead.Concentration)
            if Concentration ~= -999 && Concentration ~= 0
                Ambient.BagRead.Concentration = [tempChannel.Ambient.BagRead.Concentration, max(double(Concentration), 0)]; 
                options.Engine_Modal_Amb_Bag_Test = 1; 
            else
                Ambient.BagRead.Concentration = [tempChannel.Ambient.BagRead.Concentration, -999];
            end
        else
            % ES changed on 3/27/2018 to allow Concentration = 0
            % if Concentration ~= -999 && Concentration ~= 0
            if Concentration ~= -999
                Ambient.BagRead.Concentration = max(double(Concentration), 0); 
                options.Engine_Modal_Amb_Bag_Test = 1; 
            else
                Ambient.BagRead.Concentration = -999; 
            end
        end

        % Range
        if ~isempty(tempChannel.Ambient) && ~isempty(tempChannel.Ambient.BagRead) && ~isempty(tempChannel.Ambient.BagRead.Range)
            Ambient.BagRead.Range = [tempChannel.Ambient.BagRead.Range, Range]; 
        else
            Ambient.BagRead.Range = Range;
        end

        % Range size
        if ~isempty(tempChannel.Ambient) && ~isempty(tempChannel.Ambient.BagRead) && ~isempty(tempChannel.Ambient.BagRead.RangeSize)
            Ambient.BagRead.RangeSize = [tempChannel.Ambient.BagRead.RangeSize, RangeSize]; 
        else
            Ambient.BagRead.RangeSize = RangeSize;
        end

        % Phase
        if ~isempty(tempChannel.Ambient) && ~isempty(tempChannel.Ambient.BagRead) && ~isempty(tempChannel.Ambient.BagRead.Phase)
            Ambient.BagRead.Phase = [tempChannel.Ambient.BagRead.Phase, Phase]; 
        else
            Ambient.BagRead.Phase = Phase;
        end 
        
        % Update the tempChannel.Ambient field with the latest values
        if strcmpi(Writer, 'CO_h_Bag_Dilute')
            datastream('CO_h_Bag_Dilute').Ambient = Ambient; 
        end
...
% ES added 10/24/2017
% Deal with the case when HHC or HC don't have a bag read ambient value. 
if datastream.isKey('HHC_Bag_Dilute'), HHCChannel = datastream('HHC_Bag_Dilute'); end
if datastream.isKey('HC_Bag_Dilute'), HCChannel = datastream('HC_Bag_Dilute'); end

% Assume that the number of ambient values for each channel are the same
% for both HC and HHC.
try
replaceHHCIdx = d_uSetdiff(find(HHCChannel.Ambient.BagRead.Range == 0), find(HCChannel.Ambient.BagRead.Range == 0));
HHCChannel.Ambient.BagRead.Concentration(replaceHHCIdx) = HCChannel.Ambient.BagRead.Concentration(replaceHHCIdx); 
HHCChannel.Ambient.BagRead.Range(replaceHHCIdx) = HCChannel.Ambient.BagRead.Range(replaceHHCIdx); 
HHCChannel.Ambient.BagRead.RangeSize(replaceHHCIdx) = HCChannel.Ambient.BagRead.RangeSize(replaceHHCIdx); 
HHCChannel.Ambient.BagRead.Phase(replaceHHCIdx) = HCChannel.Ambient.BagRead.Phase(replaceHHCIdx); 
catch
end

try
replaceHCIdx = d_uSetdiff(find(HCChannel.Ambient.BagRead.Range == 0), find(HHCChannel.Ambient.BagRead.Range == 0));
HCChannel.Ambient.BagRead.Concentration(replaceHCIdx) = HHCChannel.Ambient.BagRead.Concentration(replaceHCIdx); 
HCChannel.Ambient.BagRead.Range(replaceHCIdx) = HHCChannel.Ambient.BagRead.Range(replaceHCIdx); 
HCChannel.Ambient.BagRead.RangeSize(replaceHCIdx) = HHCChannel.Ambient.BagRead.RangeSize(replaceHCIdx); 
HCChannel.Ambient.BagRead.Phase(replaceHCIdx) = HHCChannel.Ambient.BagRead.Phase(replaceHCIdx); 
catch
end
...
while (res.Read())
    ...
    if datastream.isKey(Writer)
        tempChannel = datastream(Writer);
        Ambient = tempChannel.Ambient; % Initialize ambient structure
        ...
        % ES 3/27/2018 removed ambient bag drift correction
%         if isfield(Ambient, 'BagRead') && ~isempty(Ambient.BagRead)
%            tempidx = find((Ambient.BagRead.Range+1) == Ambient.Range); 
%            tempconc = refzero + (refspan - refzero) * (2 * Ambient.BagRead.Concentration(tempidx) - (prezero + postzero)) / (prespan + postspan - prezero - postzero);
%            Ambient.BagRead.Concentration(tempidx) = tempconc;
%         end
