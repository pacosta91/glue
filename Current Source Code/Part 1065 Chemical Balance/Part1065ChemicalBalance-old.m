...
if WetMeasurements %Also assumes diluted exhaust
    ...
    if datastream.isKey(COhAccess)
        if ~datastream('Options').Engine_Modal_Amb_Bag_Test || ~isfield(datastream(COhAccess).Ambient,'BagRead') || any(datastream(COhAccess).Ambient.BagRead.Concentration == -999)
            ...
            AmbientConc = ModeSegregation.PhaseIndices * datastream(COhAccess).Ambient.BagRead.Concentration';
        end
    ...
    % HNOx
    % ES - Commented out 10/17/2017
%     if datastream.isKey(HNOxAccess)
%         if any(datastream(HNOxAccess).Ambient.BagRead.Concentration == -999)
...
%         end
%         part1065.HNOx_m = datastream('Options').Molar_Mass.HNOx.*part1065.n_exh.*datastream('Options').delta_T.* ...
