...
if datastream.isKey('CH4_Bag_Dilute') && datastream('Options').Part_1065.IsOn
    ...
    % CH4Channel = datastream('CH4_Bag_Dilute'); % overcoming multilevel indexing restriction
    if datastream('Options').Use_HC
        ...
        if options.Engine_Modal_Amb_Bag_Test, HCChannel.Ambient.BagRead = datastream('HC_Bag_Dilute').Ambient.BagRead; end
        % end EDS addition
        ...
        if options.Engine_Modal_Amb_Bag_Test
            ...
            if (all(datastream('HC_Bag_Dilute').Ambient.BagRead.Concentration ~= -999) && all(datastream('CH4_Bag_Dilute').Ambient.BagRead.Concentration ~= -999)) 
                CH4Channel.Ambient.BagRead.Concentration = (datastream('CH4_Bag_Dilute').CH4Channel.Ambient.BagRead.Concentration - datastream('HC_Bag_Dilute').CH4Channel.Ambient.BagRead.Concentration * datastream('Options').RFPF_C2H6) / ...
                    (1 - datastream('Options').RFPF_C2H6 * datastream('Options').RF_CH4);
                CH4Channel.Ambient.BagRead.Concentration = max(CH4Channel.Ambient.BagConc, 0);
            end 
        end
        % end EDS addition
