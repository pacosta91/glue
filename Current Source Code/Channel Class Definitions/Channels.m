...
%         end
        ...
        function [CArray, HeaderTable, EmissionsTable, BSFCTable, PMTable, AmbientTable, VZSTable, DriftTable, saveasNamec, saveasNames] = MakeReports(obj, ModeSegregation, pathname)
            ...
            AmbientTable = obj.MakeAmbientTable(ModeSegregation);
            ...
            try
                success = xlswc(saveasNamec,'Vertical',CompositeArray,{});
            catch
            end

            maxIterations = 10; iterations = 0;
        ...
        function StreamingArray = MakeStreamingArray(obj)
            ...
            if strcmp(obj.Values('Options').Option_Panel,'Network')
                % fid = fopen([obj.Values('Options').Network_Path 'Settings Files\Standard Channel List.txt']);
                fid = fopen([obj.Values('Options').Network_Path 'Settings Files\Standard Channel List With Range.txt']);
            else
                % fid = fopen([LOCALPATH '\Settings Files\Standard Channel List.txt']);
                fid = fopen([LOCALPATH '\Settings Files\Standard Channel List With Range.txt']);
            end
            k = 1;
            while ~feof(fid)
                DataMill = stringread(fgetl(fid),',');
                ...
                if length(DataMill)==1
                    ...
                    if obj.isKey(sname)
                        if instr(name,'Bench') && ~instr(name,'°') && ~instr(name,'Mol')
                            ...
                            if ~obj.isKey(sname) || isempty(obj.Values(sname).StreamingData.(DataGet)) %This makes sense to have here, even if an analyzer does exist, there is a possibilty of the chemical balance not being performed
                                if strcmp(units,'')
                    ...
                    if obj.isKey(sname)
                        ...
                        switch sname
                            ...
                            case 'BS_MSS'
                                ...
                                % PM data is scary...
                                if obj.isKey('MSS_CC') && abs(mean(obj.Values('MSS_CC').StreamingData))>0
        ...
        function HeaderTable = MakeHeaderTable(obj)
            ...
            HeaderTable{1,1} = ['SGS - Environmental Testing Center          Reviewed By: ' TESTENGINEER];
...
%                     case 14, HeaderTable{13,4} = 'Other Billable Causes';
                    ...
                    for m = 1:length(SPECIES)
                        ...
                        if obj.isKey([SPECIES{m} '_' BENCHES{k}])  && obj.Values([SPECIES{m} '_' BENCHES{k}]).IsOn
                            if strcmp(SPECIES{m},'CO_h') && Single_CO
                                ...
                                for n = 2:size(obj.Values('Time').ModeCompositeData,1)+1
                                    if obj.Values('Options').([BENCHES{k} '_CO_l_InUse'])
                                        try EmissionsTable{n,count86} = obj.Values(['CO_l_' BENCHES{k}]).ModeCompositeData.Part86BrakeSpecificMass(n-1); catch EmissionsTable{n,count86} = []; end
                                        try EmissionsTable{n,count1065} = obj.Values(['CO_l_' BENCHES{k}]).ModeCompositeData.Part1065BrakeSpecificMass(n-1); catch EmissionsTable{n,count1065} = []; end
                                    else
                                        try EmissionsTable{n,count86} = obj.Values(['CO_h_' BENCHES{k}]).ModeCompositeData.Part86BrakeSpecificMass(n-1); catch EmissionsTable{n,count86} = []; end
                                        try EmissionsTable{n,count1065} = obj.Values(['CO_h_' BENCHES{k}]).ModeCompositeData.Part1065BrakeSpecificMass(n-1); catch EmissionsTable{n,count1065} = []; end 
                                    end
                                end
                                count86 = count86+1;
                            ...
                            elseif ~strcmp(SPECIES{m},'NMHC')
                                ...
                                try EmissionsTable{1,count86} = [obj.Values([SPECIES{m} '_' BENCHES{k}]).Name ' (' obj.Values([SPECIES{m} '_' BENCHES{k}]).Current_Units{3} ')']; catch EmissionsTable{1,count86} = [SPECIES{m} '_' BENCHES{k}]; end 
                                try EmissionsTable{1,count1065} = ['MF_' obj.Values([SPECIES{m} '_' BENCHES{k}]).Name ' (' obj.Values([SPECIES{m} '_' BENCHES{k}]).Current_Units{3} ')']; catch EmissionsTable{1,count1065} = ['MF_' SPECIES{m} '_' BENCHES{k}]; end 
                                try EmissionsTable(2:end,count86) = num2cell(obj.Values([SPECIES{m} '_' BENCHES{k}]).ModeCompositeData.Part86BrakeSpecificMass); catch EmissionsTable(2:end,count86) = {[]}; end
                                try EmissionsTable(2:end,count1065) = num2cell(obj.Values([SPECIES{m} '_' BENCHES{k} Corrected]).ModeCompositeData.Part1065BrakeSpecificMass); catch EmissionsTable(2:end,count1065) = {[]}; end 
                                count86 = count86+1;
                                count1065 = count1065+1;
                            else
                                try EmissionsTable{1,count1065} = ['MF_' obj.Values([SPECIES{m} '_' BENCHES{k}]).Name ' (' obj.Values([SPECIES{m} '_' BENCHES{k}]).Current_Units{3} ')']; catch EmissionsTable(1,count1065) = ['MF_' SPECIES{m} '_' BENCHES{k}]; end
                                try EmissionsTable(2:end,count1065) = num2cell(obj.Values([SPECIES{m} '_' BENCHES{k}]).ModeCompositeData.Part1065BrakeSpecificMass); catch EmissionsTable(2:end,count1065) = {[]}; end
                                count1065 = count1065+1;
                            end
                        end
                    end
        ...
        function AmbientTable = MakeAmbientTable(obj, ModeSegregation)
            global SPECIES;

            count = 0;
            for k = 1:length(SPECIES)
                if obj.isKey([SPECIES{k} '_Bag_Dilute']) && ~isempty(obj.Values([SPECIES{k} '_Bag_Dilute']).Ambient) && ...
                        numel(fieldnames(obj.Values([SPECIES{k} '_Bag_Dilute']).Ambient)) > 1 % ES added to deal with cases when Ambient only contains BagRead
                    count = count+1;
                end
                if obj.Values('Options').Part_1065.IsOn
            ...
            if count ~= 0
                ...
                if obj.Values('Options').Part_1065.IsOn && obj.Values('Options').Engine_Modal_Amb_Bag_Test % Show Bag Read Ambients instead of normal 1065 ambients
                    PhaseAndRange = {};
                    for i = 1:ModeSegregation.nPhases
                        PhaseAndRange(end+1:end+2,1) = {['Phase' num2str(i) ' Amb Bag Conc']; ['Phase' num2str(i) ' Range']};
                    end
                    AmbientTable = cell(2 + 2*ModeSegregation.nPhases, count + 1);
                    AmbientTable(:,1) = vertcat({'Analyzer'; 'Units'}, PhaseAndRange);
                    position = 2;
                    for k = 1:length(SPECIES)
                        if obj.isKey([SPECIES{k} '_Bag_Dilute'])
                            BagConc = []; Range = [];
                            try BagConc = obj.Values([SPECIES{k} '_Bag_Dilute']).Ambient.BagRead.Concentration; catch ME; end
                            BagConc_length = length(BagConc);
                            try Range = obj.Values([SPECIES{k} '_Bag_Dilute']).Ambient.BagRead.Range; catch ME; end
                            ...
                            for i = 1:ModeSegregation.nPhases
                                if BagConc_length >= i && BagConc(i) ~= -999, AmbientTable{2*i+1, position} = BagConc(i); else AmbientTable{2*i+1, position} = '---'; end
                                if Range_length >= i && Range(i) ~= 0, AmbientTable{2*i+2, position} = Range(i); else AmbientTable{2*i+2, position} = '---'; end
                            end
                            position = position+1;
                        end
                    end

                elseif obj.Values('Options').Part_1065.IsOn && obj.Values('Options').Part_1065.DriftCorrection
    ...
    methods (Static)
        ...
        function PMData = GetPMData(ModeSegregation)
            ...
            % ES 7/7/2016 - We could commented out the below if-statment to include PM
            % Data in all cases when specified
