...
formatSpec = [repmat('%s, ', 1, cols-1) '%s\r\n']; 
fid = fopen(filename,'w');
if fid > 0
    for i = 1:rows
        fprintf(fid, formatSpec, headers{:,i});
    end
    fclose(fid);
end
...
%
dlmwrite(filename, m,'-append','delimiter',',','precision',12,'roffset',r,'coffset',c);